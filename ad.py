...
import json
import requests
from httpserver import random_with_N_digits
from same_spider.secret import header


def get_latest_ugc_author_uids():
    try:
        res = requests.get('http://localhost:9200/_sql?sql=select * from same/music order by timestamp desc limit 100')
...
def send_ad(msg):
    ...
    for to_uid in get_latest_ugc_author_uids():
        ...
        try:
            res = requests.post(fetch_url, headers=header, json=body, verify=False)
            if res.status_code == 200:
                print res.text
            else:
